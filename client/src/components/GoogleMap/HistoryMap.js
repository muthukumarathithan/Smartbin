...
class HistoryMap extends Component{
    ...
    var start_marker = new window.google.maps.Marker({
      map: map,
    });
    this.setState({loaded:true});
    start_marker.setPosition(new window.google.maps.LatLng(this.props.history[0].latitude,this.props.history[0].longitude))
    start_marker.setIcon({
        url: `static/media/vehicles/flag/start.png`, 
        anchor: new window.google.maps.Point(33,33),
      })

   map.setCenter(new window.google.maps.LatLng(this.props.history[0].latitude, this.props.history[0].longitude));
   image.src = amb;
   this.drawPolyline();
...
export default connect(store=>({history:store.history }))(withStyles(styles)(HistoryMap));
