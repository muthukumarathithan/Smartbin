import React, { useState } from "react";
import Dot from '../Sidebar/components/CustomDot'
import {
  Grid,
  CircularProgress,
  Typography,
  Button,
  Tabs,
  Tab,
  TextField,
  Fade,
} from "@material-ui/core";
import { withRouter } from "react-router-dom";
import classnames from "classnames";
import moment from "moment";
import {connect} from 'react-redux';

// styles
import useStyles from "./styles";

// logo
import driver from "../../images/avatar.jpg";
import google from "../../images/driver.jpg";

// context
import { useUserDispatch, loginUser } from "../../context/UserContext";
import HistoryForm from "./HistoryForm";
import LocationAddress from "./LocationAddress";

//redux
import {setMapView} from '../../store/actions';



function Vehicleinfo(props) {
  var classes = useStyles();
  var vehicle = props.vehicle;

  // global
  var userDispatch = useUserDispatch();

  // local
  var [isLoading, setIsLoading] = useState(false);
  var [error, setError] = useState(null);
  var [activeTabId, setActiveTabId] = useState(0);
  var [nameValue, setNameValue] = useState("");
  var [loginValue, setLoginValue] = useState("");
  var [passwordValue, setPasswordValue] = useState("");

  return (
    <Grid container className={classes.container}>
      <div className={classes.formContainer}>
        <div className={classes.form}>
          <Tabs
            value={activeTabId}
            onChange={(e, id) => setActiveTabId(id)}
            indicatorColor="primary"
            textColor="primary"
            centered
            classes={{ root: classes.MuiTabsCentered }}
          >
            <Tab label="Details" classes={{ root: classes.tab }} />
            <Tab label="History" classes={{ root: classes.tab }} />
          </Tabs>
          {activeTabId === 0 && (
            <React.Fragment>
              <Typography variant="h3" style={{marginTop:'10px', display:'flex', alignItems:'center' }} className={classes.greeting}>
                 <Dot size="xlarge" color={vehicle.color} /><span style={{margin:'0px 5px'}}>{vehicle.vehicle_reg_no} </span>
                  <Button disabled={vehicle.status !== 'idle' ? 'disabled':''} style={{padding:'revert', color:'white', background:vehicle.status === 'online' ? vehicle.color:'#B9B9B9'}} onClick={()=>(props.setMapView(4))} >Live Track</Button>
              </Typography>
              <div className={classes.formDividerContainer}>
                <div className={classes.formDivider} />
              </div>
              <div className={classes.description}>
                <img src={driver} alt="driver" className={classes.driverIcon} />
                <div className={classes.driverDesc}>
                  <Typography variant="h6" >
                    Driving by
                 </Typography>
          <span className={classes.detailsText}>Not Mentioned</span>
                 <span className={classes.detailsText}> Not Mentioned</span>
                 <span className={classes.detailsText}> Not Mentioned</span>
                </div>
                <div className={classes.locationDesc}>
                  <Typography variant="h6" >
                      Location
                  </Typography>
                  <span className={classes.detailsText}> Sipcot IT Park, Siruseri, Chennai </span>

                </div>
              </div>

              <Typography variant="h7" className={classes.greeting}>
                 Last Updated: {moment(vehicle.l_created_date).format('DD-MM-YYYY hh:mm:ss A')}
              </Typography>

              <div className={classes.formDividerContainer}>
                <div className={classes.formDivider} />
              </div>

              <div className={classes.indicatorIcon}>
                    <a href="javascript:;" className={classes.iconBtn}>
                        <svg height="20pt" style={{fill:vehicle.ignition_status ? 'green':'red'}} viewBox="-162 1 511 511.99931" width="25pt" xmlns="http://www.w3.org/2000/svg"><path d="m179.699219 247.46875h-34.132813v-170.667969c0-14.140625-11.460937-25.601562-25.597656-25.601562v-34.132813c0-9.425781-7.644531-17.066406-17.070312-17.066406h-17.066407c-9.425781 0-17.066406 7.640625-17.066406 17.066406v34.132813c-14.136719 0-25.597656 11.460937-25.597656 25.601562v170.664063h-34.136719c-4.710938 0-8.53125 3.820312-8.53125 8.535156v51.199219c0 4.714843 3.820312 8.535156 8.53125 8.535156h17.070312v42.664063c0 4.714843 3.820313 8.535156 8.53125 8.535156h8.535157v76.800781c0 14.136719 11.460937 25.597656 25.597656 25.597656h17.066406v8.535157c0 4.710937 3.820313 8.53125 8.535157 8.53125 4.710937 0 8.53125-3.820313 8.53125-8.53125v-8.535157h8.535156v17.066407c0 4.714843-3.820313 8.535156-8.535156 8.535156h-8.53125c-4.714844 0-8.535157 3.820312-8.535157 8.53125 0 4.714844 3.820313 8.535156 8.535157 8.535156h8.53125c14.140624 0 25.601562-11.460938 25.601562-25.601562v-18.636719c10.191406-3.601563 17.019531-13.21875 17.066406-24.027344v-76.800781h8.53125c4.714844 0 8.535156-3.820313 8.535156-8.535156v-42.664063h17.066407c4.710937 0 8.535156-3.820313 8.535156-8.535156v-51.199219c0-4.714844-3.820313-8.53125-8.535156-8.53125zm-93.867188-230.402344h17.066407v34.132813h-17.066407zm-25.597656 59.734375c0-4.714843 3.820313-8.535156 8.53125-8.535156h51.203125c4.710938 0 8.53125 3.820313 8.53125 8.535156v170.664063h-68.265625zm68.265625 341.332031h-68.265625v-17.066406h68.265625zm-8.53125 34.132813h-51.203125c-4.710937 0-8.53125-3.820313-8.53125-8.53125v-8.535156h68.265625v8.535156c0 4.710937-3.820312 8.53125-8.53125 8.53125zm8.53125-68.265625h-68.265625v-17.066406h68.265625zm17.066406-34.132812h-102.398437v-34.132813h102.398437zm25.601563-51.199219h-153.601563v-34.132813h153.601563zm0 0" /></svg>
                              <div>
                                Ignition
                              </div>
                            </a>
                            <a href="javascript:;" className={classes.iconBtn}>
                              {/*?xml version="1.0" encoding="iso-8859-1"?*/}
                              {/* Generator: Adobe Illustrator 19.0.0, SVG Export Plug-In . SVG Version: 6.00 Build 0)  */}
                              <svg height="20pt" width="25pt" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" style={{enableBackground: 'new 0 0 512 512'}} xmlSpace="preserve">
                                <path d="M464,0H336v32h128c8.837,0,16,7.163,16,16v352c0,8.837-7.163,16-16,16H48c-8.837,0-16-7.163-16-16V48
                      c0-8.837,7.163-16,16-16h128V0H48C21.49,0,0,21.49,0,48v352c0,26.51,21.49,48,48,48h416c26.51,0,48-21.49,48-48V48
                      C512,21.49,490.51,0,464,0z" />
                                <rect x={32} y={480} width={96} height={32} />
                                <rect x={384} y={480} width={96} height={32} />
                                <path d="M123.595,74.375c-3.404,2.266-6.575,4.863-9.467,7.753c-24.183,24.179-24.186,63.385-0.007,87.568
                      c2.9,2.9,6.08,5.505,9.495,7.776l53.488,35.616c-0.62,3.607-0.989,7.253-1.104,10.912c0.123,3.523,0.476,7.035,1.056,10.512
                      l-53.408,35.664c-28.442,18.94-36.145,57.35-17.205,85.792s57.35,36.145,85.792,17.205c6.833-4.55,12.693-10.415,17.237-17.253
                      l35.328-53.056c3.701,0.647,7.444,1.026,11.2,1.136c3.002-0.125,5.994-0.429,8.96-0.912l35.2,52.8
                      c18.882,28.491,57.285,36.281,85.776,17.4c28.491-18.882,36.281-57.285,17.4-85.776c-4.555-6.874-10.438-12.768-17.304-17.336
                      l-51.2-34.192c0.696-3.959,1.086-7.965,1.168-11.984c-0.089-4.165-0.506-8.317-1.248-12.416l51.2-34.128
                      c28.453-18.971,36.139-57.416,17.167-85.869c-18.971-28.453-57.416-36.139-85.869-17.167c-6.809,4.54-12.65,10.385-17.187,17.196
                      l-35.408,53.264c-2.867-0.456-5.756-0.749-8.656-0.88c-3.653,0.116-7.294,0.484-10.896,1.104l-35.632-53.488
                      C190.519,63.141,152.07,55.421,123.595,74.375z M182.928,338.128c-5.547,8.346-14.907,13.356-24.928,13.344
                      c-7.95,0.067-15.587-3.091-21.168-8.752c-11.648-11.608-11.68-30.462-0.072-42.11c1.426-1.431,2.994-2.713,4.68-3.826
                      l46.672-31.152c6.471,10.534,15.286,19.432,25.76,26L182.928,338.128z M368.368,296.816c13.732,9.111,17.478,27.63,8.367,41.362
                      c-5.566,8.389-14.987,13.406-25.055,13.342c-10.016,0.007-19.371-4.995-24.928-13.328l-30.32-45.552
                      c10.865-6.367,20.079-15.203,26.896-25.792L368.368,296.816z M326.768,109.312c5.559-8.32,14.906-13.314,24.912-13.312
                      c7.95-0.067,15.587,3.091,21.168,8.752c11.688,11.718,11.663,30.693-0.055,42.381c-1.387,1.384-2.907,2.628-4.537,3.715
                      L323.2,180.8c-6.852-10.532-16.088-19.302-26.96-25.6L326.768,109.312z M256,176c26.51,0,48,21.49,48,48c0,26.51-21.49,48-48,48
                      c-26.51,0-48-21.49-48-48C208,197.49,229.49,176,256,176z M182.832,109.36l31.2,46.848c-10.486,6.505-19.329,15.342-25.84,25.824
                      l-46.816-31.168c-13.781-9.166-17.521-27.769-8.355-41.549c1.091-1.64,2.34-3.168,3.731-4.563
                      c5.581-5.661,13.218-8.819,21.168-8.752C167.938,96,177.29,101.015,182.832,109.36z" />
                                <rect x={384} y={208} width={64} height={32} />
                                <rect x={64} y={208} width={64} height={32} />
                                <rect x={208} width={32} height={32} />
                                <rect x={272} width={32} height={32} />
                              </svg>
                              <div>
                                AC
                              </div>
                            </a>
                            <a href="javascript:;" className={classes.iconBtn}>
                              <svg xmlns="http://www.w3.org/2000/svg" height="20pt" width="25pt" version="1.1" viewBox="0 0 640 640" style={{fill:vehicle.main_btry_status ? 'green':'red',enableBackground: 'new 0 0 512 512'}}>
                                <g id="surface1">
                                  <path d="M 629.332031 170.664062 L 576 170.664062 L 576 138.667969 C 576 132.773438 571.226562 128 565.332031 128 L 480 128 C 474.105469 128 469.332031 132.773438 469.332031 138.667969 L 469.332031 170.664062 L 170.664062 170.664062 L 170.664062 138.667969 C 170.664062 132.773438 165.890625 128 160 128 L 74.667969 128 C 68.773438 128 64 132.773438 64 138.667969 L 64 170.664062 L 10.667969 170.664062 C 4.773438 170.664062 0 175.4375 0 181.332031 L 0 245.332031 C 0 251.226562 4.773438 256 10.667969 256 L 21.332031 256 L 21.332031 629.332031 C 21.332031 635.226562 26.109375 640 32 640 L 608 640 C 613.890625 640 618.667969 635.226562 618.667969 629.332031 L 618.667969 256 L 629.332031 256 C 635.226562 256 640 251.226562 640 245.332031 L 640 181.332031 C 640 175.4375 635.226562 170.664062 629.332031 170.664062 Z M 490.664062 149.332031 L 554.667969 149.332031 L 554.667969 170.664062 L 490.664062 170.664062 Z M 85.332031 149.332031 L 149.335938 149.332031 L 149.335938 170.664062 L 85.332031 170.664062 Z M 597.335938 618.667969 L 42.664062 618.667969 L 42.664062 256 L 597.335938 256 Z M 618.667969 234.667969 L 21.332031 234.667969 L 21.332031 192 L 618.667969 192 Z M 618.667969 234.667969 " style={{stroke: 'none', fillRule: 'nonzero',  fillOpacity: 1}} />
                                  <path d="M 225.175781 490.601562 L 306.945312 481.515625 L 288.167969 584.757812 C 287.261719 589.761719 290.007812 594.71875 294.742188 596.585938 C 295.992188 597.085938 297.324219 597.339844 298.667969 597.335938 C 302.28125 597.328125 305.648438 595.5 307.617188 592.457031 L 424.953125 411.125 C 428.148438 406.175781 426.726562 399.574219 421.78125 396.375 C 419.722656 395.042969 417.265625 394.460938 414.824219 394.730469 L 337.480469 403.328125 L 383.042969 303.0625 C 385.46875 297.695312 383.082031 291.371094 377.714844 288.949219 C 376.335938 288.324219 374.84375 288.003906 373.328125 288.003906 L 277.335938 288.003906 C 272.601562 288.003906 268.4375 291.125 267.105469 295.660156 L 213.769531 476.992188 C 212.109375 482.640625 215.34375 488.574219 220.996094 490.234375 C 222.347656 490.628906 223.769531 490.757812 225.175781 490.601562 Z M 285.3125 309.332031 L 356.777344 309.332031 L 310.292969 411.582031 C 307.855469 416.949219 310.226562 423.273438 315.585938 425.707031 C 317.335938 426.503906 319.261719 426.8125 321.171875 426.601562 L 394.824219 418.421875 L 318.710938 536.054688 L 330.492188 471.242188 C 331.546875 465.445312 327.703125 459.890625 321.910156 458.835938 C 320.894531 458.652344 319.851562 458.613281 318.824219 458.730469 L 238.75 467.625 Z M 285.3125 309.332031 " style={{stroke: 'none', fillRule: 'nonzero',  fillOpacity: 1}} />
                                  <path d="M 480 64 L 512 64 L 512 96 C 512 101.890625 516.777344 106.664062 522.664062 106.664062 C 528.558594 106.664062 533.335938 101.890625 533.335938 96 L 533.335938 64 L 565.332031 64 C 571.226562 64 576 59.222656 576 53.335938 C 576 47.441406 571.226562 42.664062 565.332031 42.664062 L 533.335938 42.664062 L 533.335938 10.667969 C 533.335938 4.773438 528.558594 0 522.664062 0 C 516.777344 0 512 4.773438 512 10.667969 L 512 42.664062 L 480 42.664062 C 474.105469 42.664062 469.332031 47.441406 469.332031 53.335938 C 469.332031 59.222656 474.105469 64 480 64 Z M 480 64 " style={{stroke: 'none', fillRule: 'nonzero',  fillOpacity: 1}} />
                                  <path d="M 74.667969 64 L 160 64 C 165.894531 64 170.667969 59.222656 170.667969 53.335938 C 170.667969 47.441406 165.894531 42.664062 160 42.664062 L 74.667969 42.664062 C 68.773438 42.664062 64 47.441406 64 53.335938 C 64 59.222656 68.773438 64 74.667969 64 Z M 74.667969 64 " style={{stroke: 'none', fillRule: 'nonzero',  fillOpacity: 1}} />
                                </g>
                              </svg>
                              <div>
                                External
                              </div>
                            </a>
                            <a href="javascript:;" className={classes.iconBtn}>
                              <svg version="1.1" id="Capa_1" height="20pt" width="25pt" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 411.347 411.347" style={{fill:vehicle.box ? 'red':'green',enableBackground: 'new 0 0 411.347 411.347'}} xmlSpace="preserve">
                                <g>
                                  <path d="M285.32,217.557c-4.051-4.051-9.437-6.282-15.165-6.282c-1.389,0-2.757,0.137-4.092,0.393l-94.35-94.349l18.283-18.283
                      c2.929-2.929,2.929-7.677,0-10.606l-4.627-4.627l69.412-69.412c2.079-2.079,2.751-5.181,1.72-7.935
                      c-1.032-2.753-3.577-4.65-6.511-4.851c-39.83-2.727-78.87,11.595-107.485,39.333l-4.648-4.648
                      c-1.406-1.406-3.314-2.197-5.303-2.197s-3.897,0.79-5.303,2.197l-74.485,74.485c-2.929,2.929-2.929,7.677,0,10.606l7.175,7.175
                      l-13.146,13.146l-7.175-7.175c-2.929-2.929-7.678-2.929-10.606,0L2.197,161.344C0.79,162.751,0,164.659,0,166.648
                      s0.79,3.897,2.197,5.303l52.14,52.139c1.464,1.465,3.384,2.197,5.303,2.197s3.839-0.732,5.303-2.197l26.816-26.816
                      c1.407-1.406,2.197-3.314,2.197-5.303c0-1.989-0.79-3.897-2.197-5.303l-7.178-7.178l13.146-13.146l7.178,7.178
                      c1.406,1.406,3.314,2.197,5.303,2.197s3.897-0.79,5.303-2.197l18.283-18.283l94.356,94.356c-1.302,6.759,0.658,14.028,5.881,19.251
                      l111.896,111.895c4.051,4.051,9.437,6.282,15.165,6.282c5.729,0,11.115-2.231,15.166-6.282l20.958-20.958c0,0,0,0,0,0
                      c8.361-8.362,8.361-21.968,0-30.331L285.32,217.557z M231.359,16.599l-56.597,56.598l-21.654-21.655
                      C174.412,30.968,202.144,18.697,231.359,16.599z M59.64,208.18l-41.533-41.533l16.21-16.21l41.533,41.533L59.64,208.18z
                      M73.975,168.883l-16.573-16.573l13.146-13.146l16.573,16.573L73.975,168.883z M110.208,157.612l-41.533-41.533l63.878-63.878
                      l41.533,41.533L110.208,157.612z M161.107,127.926l91.757,91.756l-16.706,16.706l-91.757-91.756L161.107,127.926z M386.608,349.176
                      l-20.958,20.958c-1.218,1.218-2.836,1.888-4.559,1.888c-1.722,0-3.341-0.671-4.559-1.888L244.639,258.239
                      c-2.512-2.512-2.513-6.599-0.005-9.113l20.968-20.968c1.217-1.214,2.833-1.883,4.553-1.883c1.722,0,3.341,0.671,4.559,1.888
                      l111.895,111.894C389.122,342.572,389.122,346.663,386.608,349.176z" />
                                  <path d="M248.499,170.211c-2.929,2.929-2.929,7.678,0,10.607c2.93,2.929,7.678,2.929,10.607,0l16.02-16.02
                      c19.011-19.011,28.245-44.863,25.335-70.926c-1.744-15.617,4.279-31.152,16.113-41.556c3.396-2.986,7.192-5.456,11.272-7.354
                      l-6.163,19.756c-2.553,8.18-1.767,16.864,2.214,24.453c3.979,7.589,10.677,13.173,18.856,15.725
                      c16.885,5.269,34.909-4.185,40.179-21.071l6.366-20.41c4.458,7.255,6.904,15.593,7.042,24.344
                      c0.207,13.131-4.792,25.476-14.075,34.76c-5.454,5.453-11.914,9.407-19.201,11.753c-13.575,4.37-25.942,11.87-35.763,21.691
                      l-39.191,39.191c-2.929,2.929-2.929,7.677,0,10.606c1.465,1.465,3.385,2.197,5.304,2.197s3.839-0.732,5.304-2.197l39.191-39.191
                      c8.145-8.145,18.432-14.376,29.751-18.019c9.441-3.039,18.159-8.373,25.212-15.425c12.181-12.181,18.739-28.377,18.467-45.604
                      c-0.271-17.227-7.353-33.221-19.938-45.038c-1.884-1.768-4.545-2.443-7.042-1.785c-2.498,0.657-4.482,2.554-5.252,5.02
                      l-10.494,33.639c-2.805,8.99-12.404,14.023-21.392,11.219c-4.355-1.359-7.921-4.332-10.039-8.372
                      c-2.119-4.041-2.538-8.664-1.179-13.019l10.374-33.254c0.773-2.482,0.208-5.188-1.497-7.151c-1.704-1.964-4.31-2.903-6.869-2.485
                      c-11.691,1.905-22.528,7.008-31.341,14.755c-15.51,13.636-23.403,34.005-21.117,54.486c2.402,21.507-5.265,42.885-21.034,58.655
                      L248.499,170.211z" />
                                  <path d="M211.795,271.471c-19.01,19.01-28.244,44.861-25.334,70.926c1.744,15.616-4.28,31.151-16.114,41.556
                      c-3.396,2.986-7.193,5.456-11.271,7.354l6.163-19.756c2.552-8.18,1.766-16.864-2.214-24.453
                      c-3.98-7.589-10.677-13.173-18.857-15.725c-8.181-2.552-16.865-1.766-24.453,2.214c-7.589,3.98-13.173,10.677-15.725,18.856
                      l-6.367,20.41c-4.459-7.256-6.904-15.592-7.042-24.345c-0.207-13.131,4.792-25.476,14.076-34.76
                      c5.453-5.453,11.913-9.407,19.201-11.752c13.575-4.369,25.941-11.87,35.762-21.691l39.819-39.82c2.929-2.929,2.929-7.678,0-10.606
                      c-2.929-2.929-7.678-2.929-10.606,0l-39.819,39.82c-8.145,8.145-18.433,14.376-29.751,18.019
                      c-9.441,3.039-18.16,8.373-25.211,15.425c-12.181,12.181-18.739,28.376-18.468,45.603c0.271,17.227,7.353,33.222,19.939,45.039
                      c1.883,1.768,4.546,2.443,7.042,1.785c2.498-0.657,4.482-2.554,5.251-5.02l10.494-33.639c1.358-4.355,4.332-7.92,8.372-10.039
                      c4.04-2.119,8.664-2.538,13.02-1.179c4.355,1.359,7.921,4.332,10.04,8.372s2.538,8.664,1.179,13.019l-10.374,33.254
                      c-0.774,2.482-0.208,5.188,1.496,7.151c1.437,1.655,3.51,2.583,5.663,2.583c0.401,0,0.804-0.032,1.207-0.098
                      c11.69-1.905,22.528-7.008,31.34-14.755c15.51-13.637,23.404-34.005,21.117-54.485c-2.401-21.507,5.266-42.886,21.034-58.655
                      c2.929-2.929,2.929-7.677,0-10.606C219.473,268.542,214.725,268.542,211.795,271.471z" />
                                </g>
                              </svg>
                              <div>
                                Tamper
                              </div>
                            </a>
                            <a href="javascript:;" className={classes.iconBtn}>
                              <svg height="20pt" width="25pt" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 339.452 339.452" style={{enableBackground: 'new 0 0 339.452 339.452'}} xmlSpace="preserve">
                                <g>
                                  <g>
                                    <path d="M325.452,280.557h-4.72c0.048-0.666,0.048-1.334,0-2v-252c0-7.732-6.268-14-14-14h-136.8c-7.732,0-14,6.268-14,14v104
                      c-13.025,3.16-24.423,11.024-32,22.08c-9.292,13.387-12.164,30.208-7.84,45.92c5.76,22.08,6.48,51.36-5.44,63.52
                      c-6.863,6.083-16.517,7.897-25.12,4.72c-9.254-1.465-17.155-7.474-21.04-16c-12.72-29.36,19.04-84.88,19.12-85.12
                      c1.92-2.88,46.16-71.6,0.96-112c-1.84-1.92-45.44-48-79.12-43.52c-3.424,0.442-5.842,3.576-5.4,7c0.442,3.424,3.576,5.842,7,5.4
                      c8.094-0.486,16.162,1.316,23.28,5.2l-8.96,10.8c-4.856,6.017-3.914,14.831,2.103,19.687c0.098,0.079,0.197,0.157,0.297,0.233
                      l30.08,24c2.473,1.982,5.551,3.055,8.72,3.04h1.2c3.688-0.416,7.058-2.288,9.36-5.2l8.72-10.96c27.04,34.56-8,89.6-8.88,90.48
                      c-1.44,2.48-35.04,61.28-20,96c5.56,12.056,16.739,20.568,29.84,22.72c4.035,0.978,8.169,1.488,12.32,1.52
                      c8.97,0.27,17.654-3.175,24-9.52c16-16.72,14.72-50.8,8.48-74.96c-6.637-22.24,5.95-45.662,28.16-52.4v135.36
                      c-0.048,0.666-0.048,1.334,0,2h-4c-7.73-0.177-14.14,5.946-14.316,13.676c-0.003,0.108-0.004,0.216-0.004,0.324v21.04
                      c-0.002,7.732,6.264,14.002,13.996,14.004c0.108,0,0.216-0.001,0.324-0.004h173.68c7.732,0,14-6.268,14-14v-21.04
                      C339.452,286.826,333.184,280.557,325.452,280.557z M72.652,61.917l-8.88,10.96c-0.339,0.412-0.829,0.671-1.36,0.72
                      c-0.514,0.067-1.034-0.078-1.44-0.4l-30.08-24c-0.762-0.778-0.762-2.022,0-2.8l8.88-11.04c0.339-0.412,0.829-0.671,1.36-0.72
                      c0.514-0.067,1.034,0.078,1.44,0.4l30.08,24c0.419,0.333,0.68,0.826,0.72,1.36C73.367,60.985,73.104,61.541,72.652,61.917z
                      M167.772,26.557c0-1.105,0.895-2,2-2h137.12c1.105,0,2,0.895,2,2v252c0,1.105-0.895,2-2,2h-136.96
                      c-1.101,0.088-2.065-0.733-2.154-1.834c-0.004-0.055-0.007-0.111-0.006-0.166V26.557z M327.772,315.597c0,1.105-0.895,2-2,2h-174
                      c-1.105,0-2-0.895-2-2v-21.04c0-1.105,0.895-2,2-2h173.68c1.09-0.177,2.118,0.564,2.294,1.654c0.019,0.114,0.027,0.23,0.026,0.346
                      V315.597z" />
                                  </g>
                                </g>
                                <g>
                                  <g>
                                    <path d="M279.772,43.597h-82.88c-7.732,0-14,6.268-14,14v82.96c0,7.732,6.268,14,14,14h82.88c7.732,0,14-6.268,14-14v-82.96
                      C293.772,49.866,287.504,43.597,279.772,43.597z M281.772,140.637c-0.043,1.073-0.926,1.921-2,1.92h-82.88c-1.105,0-2-0.895-2-2
                      v-82.96c0-1.105,0.895-2,2-2h82.88c1.105,0,2,0.895,2,2V140.637z" />
                                  </g>
                                </g>
                                <g>
                                  <g>
                                    <path d="M270.412,177.198h-64c-3.314,0-6,2.686-6,6s2.686,6,6,6h64c3.314,0,6-2.686,6-6S273.726,177.198,270.412,177.198z" />
                                  </g>
                                </g>
                              </svg>
                              <div>
                                Fuel
                              </div>
                            </a>
                        </div>
                        <div className={classes.indicatorIcon}>
   
                            <a href="javascript:;" className={classes.iconBtn}>
                              <svg version="1.1" height="20pt" width="25pt" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" style={{fill:vehicle.sos ? 'red':'green',enableBackground: 'new 0 0 512 512'}} xmlSpace="preserve">
                                <g>
                                  <g>
                                    <path d="M442.332,69.666C397.407,24.741,337.677,0,274.144,0c-63.295,0-124.337,25.538-168.809,70.316
                      c-7.086-3.715-14.379-5.561-21.939-5.561c-22.742,0-41.198,16.693-57.481,31.421c-2.86,2.587-5.563,5.031-8.192,7.294
                      c-9.856,8.297-15.854,22.284-17.346,40.453c-2.214,26.964,5.452,62.626,21.584,100.415
                      c18.184,42.595,46.663,87.105,82.319,128.673C185.62,469.024,272.641,512,322.798,512c14.542,0,26.102-3.502,34.357-10.41
                      l0.12-0.102c2.778-2.4,5.743-4.832,8.883-7.407c18.482-15.158,39.431-32.339,40.362-57.477c0.015-0.391,0.018-0.783,0.023-1.174
                      c31.518-21.132,57.735-49.706,76.031-82.983c19.248-35.012,29.423-74.637,29.423-114.591
                      C511.998,174.322,487.257,114.591,442.332,69.666z M83.396,84.757c11.945,0,24.366,9.258,36.921,27.52
                      c16.662,24.223,23.368,41.954,20.499,54.205c-1.003,4.283-3.195,8.161-6.252,11.932c-15.035-28.822-39.829-67.967-59.742-92.259
                      C77.683,85.259,80.547,84.757,83.396,84.757z M353.475,478.616c-3.217,2.638-6.257,5.131-9.207,7.678
                      c-4.5,3.732-11.922,5.705-21.47,5.705c-44.068,0-126.472-41.281-203.297-131.963c-34.385-40.085-61.753-82.808-79.146-123.55
                      c-14.878-34.852-21.997-67.144-20.044-90.925c1.032-12.573,4.695-22.093,10.313-26.807l0.096-0.081
                      c2.832-2.437,5.639-4.976,8.61-7.663c5.629-5.091,11.569-10.462,17.664-15.011c20.309,23.918,47.971,67.899,62.436,96.748
                      c-1.316,1.088-2.662,2.191-4.047,3.325c-1.34,1.097-2.697,2.208-4.068,3.34l-0.191,0.163
                      c-13.007,11.305-12.141,30.645,2.575,57.482c9.691,17.673,25.457,39.272,48.223,66.055c44.736,52.247,75.801,76.581,97.763,76.581
                      c6.667,0,12.741-2.221,17.531-6.395c0.188-0.162,0.365-0.32,0.553-0.482c26.973,15.864,66.532,47.857,88.994,74.547
                      C362.537,471.182,357.985,474.917,353.475,478.616z M380.378,452.498c-22.834-26.478-59.125-56.095-86.566-73.324
                      c7.424-5.736,14.373-9.542,22.175-9.542c12.348,0,28.385,9.329,49.024,28.516c14.689,14.001,21.931,26.691,21.521,37.715
                      C386.315,441.716,383.997,447.226,380.378,452.498z M401.937,414.291c-4.451-9.997-12.139-20.158-23.174-30.667l-0.087-0.081
                      c-24.876-23.137-44.797-33.912-62.69-33.912c-19.268,0-32.807,11.866-47.142,24.429c-1.539,1.349-3.102,2.72-4.733,4.127
                      c-1.21,1.054-2.534,1.504-4.427,1.504c-8.584,0-30.725-9.04-82.546-69.56c-21.854-25.712-36.869-46.218-45.901-62.69
                      c-8.988-16.392-11.621-28.602-7.061-32.709c1.305-1.078,2.599-2.136,3.875-3.181c3.532-2.891,7.015-5.744,10.324-8.657
                      c0.071-0.063,0.145-0.122,0.214-0.186c10.157-8.972,18.63-18.548,21.701-31.666c4.342-18.546-2.904-40.166-23.494-70.098
                      c-5.017-7.298-10.1-13.463-15.274-18.531C162.075,42.631,217.114,20,274.144,20c120.125,0,217.854,97.729,217.854,217.855
                      C491.998,308.372,458.57,373.372,401.937,414.291z" />
                                  </g>
                                </g>
                                <g>
                                  <g>
                                    <path d="M219.782,52.551c-1.827-5.212-7.536-7.958-12.745-6.129c-15.017,5.263-29.415,12.304-42.794,20.928
                      c-4.642,2.992-5.979,9.181-2.987,13.823c1.912,2.967,5.129,4.584,8.415,4.584c1.854,0,3.732-0.516,5.408-1.596
                      c12.059-7.773,25.038-14.12,38.573-18.864C218.864,63.469,221.609,57.763,219.782,52.551z" />
                                  </g>
                                </g>
                                <g>
                                  <g>
                                    <path d="M248.724,46.421c-1.01-5.431-6.236-9.012-11.659-8.004l-0.709,0.133c-5.425,1.037-8.982,6.275-7.945,11.699
                      c0.914,4.792,5.107,8.126,9.811,8.126c0.622,0,1.254-0.059,1.888-0.18l0.609-0.115C246.149,57.071,249.733,51.851,248.724,46.421z
                      " />
                                  </g>
                                </g>
                                <g>
                                  <g>
                                    <path d="M316.364,165.501c-17.501,0-31.738,14.238-31.738,31.738v57.772c0,17.5,14.238,31.738,31.739,31.738
                      c17.501,0,31.738-14.238,31.738-31.738v-57.772C348.103,179.739,333.864,165.501,316.364,165.501z M328.103,255.011
                      c0,6.473-5.266,11.738-11.738,11.738h-0.001c-6.473,0-11.738-5.266-11.738-11.738v-57.772c0-6.473,5.267-11.738,11.739-11.738
                      c6.473,0,11.738,5.266,11.738,11.738V255.011z" />
                                  </g>
                                </g>
                                <g>
                                  <g>
                                    <path d="M401.864,216.125c-8.065,0-14.627-6.562-14.627-14.627s6.561-14.626,14.626-14.626c5.209,0,10.063,2.802,12.666,7.313
                      c2.761,4.782,8.874,6.422,13.66,3.661c4.783-2.761,6.423-8.877,3.662-13.66c-6.165-10.68-17.656-17.314-29.988-17.314
                      c-19.093,0-34.626,15.533-34.626,34.626s15.533,34.627,34.626,34.627c0,0,0,0,0.001,0c8.065,0,14.626,6.561,14.626,14.626
                      s-6.561,14.627-14.626,14.627c-5.209,0-10.063-2.803-12.668-7.314c-2.761-4.782-8.877-6.422-13.66-3.66
                      c-4.783,2.761-6.422,8.877-3.66,13.66c6.166,10.68,17.656,17.314,29.988,17.314c19.093,0,34.626-15.534,34.626-34.627
                      C436.49,231.658,420.957,216.125,401.864,216.125z" />
                                  </g>
                                </g>
                                <g>
                                  <g>
                                    <path d="M230.864,216.125c-8.065,0-14.627-6.562-14.627-14.627s6.562-14.626,14.626-14.626c5.209,0,10.063,2.802,12.666,7.313
                      c2.761,4.782,8.875,6.422,13.66,3.661c4.783-2.761,6.423-8.877,3.662-13.66c-6.165-10.68-17.656-17.314-29.988-17.314
                      c-19.093,0-34.626,15.533-34.626,34.626s15.533,34.627,34.626,34.627c0,0,0,0,0.001,0c8.064,0,14.626,6.561,14.626,14.626
                      s-6.561,14.627-14.626,14.627c-5.209,0-10.063-2.803-12.668-7.314c-2.761-4.782-8.877-6.422-13.66-3.66
                      c-4.783,2.761-6.422,8.877-3.66,13.66c6.166,10.68,17.656,17.314,29.988,17.314c19.093,0,34.626-15.534,34.626-34.627
                      C265.49,231.658,249.957,216.125,230.864,216.125z" />
                                  </g>
                                </g>
                              </svg>
                              <div>
                                SOS
                              </div>
                            </a>
                            <a href="javascript:;" className={classes.iconBtn}>
                              <svg version="1.1" height="20pt" width="25pt" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 59 59" style={{fill:vehicle.gsm_signal < 10 ? 'red':'green',enableBackground: 'new 0 0 59 59'}} xmlSpace="preserve">
                                <g>
                                  <path d="M10.407,1.707c0.391-0.391,0.391-1.023,0-1.414s-1.023-0.391-1.414,0c-11.307,11.307-11.307,29.705,0,41.012
                      C9.189,41.5,9.445,41.598,9.7,41.598s0.512-0.098,0.707-0.293c0.391-0.391,0.391-1.023,0-1.414
                      C-0.12,29.363-0.12,12.234,10.407,1.707z" />
                                  <path d="M50.005,0.293c-0.391-0.391-1.023-0.391-1.414,0s-0.391,1.023,0,1.414c5.092,5.091,7.896,11.871,7.896,19.092
                      S53.683,34.8,48.591,39.891c-0.391,0.391-0.391,1.023,0,1.414c0.195,0.195,0.451,0.293,0.707,0.293s0.512-0.098,0.707-0.293
                      c5.47-5.469,8.481-12.751,8.481-20.506S55.475,5.762,50.005,0.293z" />
                                  <path d="M14.651,5.949c0.391-0.391,0.391-1.023,0-1.414s-1.023-0.391-1.414,0c-8.968,8.968-8.968,23.56,0,32.527
                      c0.195,0.195,0.451,0.293,0.707,0.293s0.512-0.098,0.707-0.293c0.391-0.391,0.391-1.023,0-1.414
                      C6.463,27.46,6.463,14.138,14.651,5.949z" />
                                  <path d="M45.763,4.535c-0.391-0.391-1.023-0.391-1.414,0s-0.391,1.023,0,1.414c8.188,8.188,8.188,21.511,0,29.699
                      c-0.391,0.391-0.391,1.023,0,1.414c0.195,0.195,0.451,0.293,0.707,0.293s0.512-0.098,0.707-0.293
                      C54.731,28.095,54.731,13.503,45.763,4.535z" />
                                  <path d="M18.893,8.778c-0.391-0.391-1.023-0.391-1.414,0c-3.206,3.206-4.972,7.475-4.972,12.021s1.766,8.814,4.972,12.021
                      c0.195,0.195,0.451,0.293,0.707,0.293s0.512-0.098,0.707-0.293c0.391-0.391,0.391-1.023,0-1.414
                      c-5.849-5.849-5.849-15.364,0-21.213C19.283,9.802,19.283,9.169,18.893,8.778z" />
                                  <path d="M41.521,8.778c-0.391-0.391-1.023-0.391-1.414,0s-0.391,1.023,0,1.414c5.849,5.849,5.849,15.364,0,21.213
                      c-0.391,0.391-0.391,1.023,0,1.414c0.195,0.195,0.451,0.293,0.707,0.293s0.512-0.098,0.707-0.293
                      c3.206-3.206,4.972-7.475,4.972-12.021S44.727,11.984,41.521,8.778z" />
                                  <path d="M29.499,14c-3.859,0-7,3.141-7,7c0,3.519,2.614,6.432,6,6.92V58c0,0.553,0.447,1,1,1s1-0.447,1-1V27.92
                      c3.386-0.488,6-3.401,6-6.92C36.499,17.141,33.359,14,29.499,14z M29.499,26c-2.757,0-5-2.243-5-5s2.243-5,5-5s5,2.243,5,5
                      S32.256,26,29.499,26z" />
                                </g>
                              </svg>
                              <div>
                                GPRS
                              </div>
                            </a>
                            <a href="javascript:;" className={classes.iconBtn}>
                              <svg   height="20pt" width="25pt" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" style={{fill:vehicle.internal_battery ,enableBackground: 'new 0 0 512 512'}} xmlSpace="preserve">
                                <g>
                                  <g>
                                    <path d="M489.657,196.47h-26.229v-40.024c0-18.48-15.035-33.515-33.515-33.515H33.515C15.035,122.931,0,137.965,0,156.445v199.109
                      c0,18.48,15.035,33.515,33.515,33.515h396.399c18.48,0,33.515-15.035,33.515-33.515V315.53h26.229
                      c12.32,0,22.343-10.023,22.343-22.343v-74.375C512,206.493,501.977,196.47,489.657,196.47z M113.377,366.726H33.515
                      c-6.16,0-11.172-5.012-11.172-11.172V156.445c0-6.16,5.012-11.172,11.172-11.172h79.862V366.726z M441.085,355.555
                      c0,6.16-5.012,11.172-11.172,11.172H135.72V145.274h294.194c6.16,0,11.172,5.012,11.172,11.172V355.555z M489.657,293.187h-26.229
                      v-74.375h26.229l0.015,74.375C489.67,293.186,489.666,293.187,489.657,293.187z" />
                                  </g>
                                </g>
                                <g>
                                  <g>
                                    <path d="M363.722,241.226c-3.3-5.21-10.202-6.761-15.416-3.459l-48.733,30.871v-45.581c0-4.073-2.216-7.823-5.786-9.787
                      c-3.567-1.963-7.923-1.829-11.365,0.35l-65.883,41.737c-5.212,3.302-6.761,10.204-3.459,15.416
                      c3.3,5.21,10.202,6.761,15.416,3.459l48.733-30.873v45.581c0,4.073,2.216,7.823,5.786,9.787c1.68,0.925,3.535,1.384,5.386,1.384
                      c2.08,0,4.158-0.581,5.979-1.734l65.883-41.736C365.475,253.339,367.025,246.438,363.722,241.226z" />
                                  </g>
                                </g>
                              </svg>
                              <div>
                                Battery
                              </div>
                            </a>
                            <a href="javascript:;" className={classes.iconBtn}>
                              <svg  xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="-4 0 966 966.24849" height="20pt" width="25pt" style={{fill:vehicle.no_of_satellites > 10 ? 'green':'red'}} >
                                <g id="surface1">
                                  <path d="M 944.710938 819.515625 L 709.601562 584.40625 C 698.703125 573.503906 681.035156 573.503906 670.136719 584.40625 L 634.46875 620.082031 L 612.300781 597.917969 L 681.226562 528.964844 C 683.5625 526.695312 685.503906 524.066406 686.988281 521.171875 C 696.871094 502.621094 693.546875 479.800781 678.789062 464.851562 L 501.5 287.609375 C 483.117188 269.308594 453.402344 269.308594 435.019531 287.609375 L 368.535156 354.097656 L 346.367188 331.984375 L 370.980469 307.386719 C 373.316406 305.121094 375.257812 302.488281 376.742188 299.601562 C 386.621094 281.050781 383.296875 258.234375 368.535156 243.273438 L 133.417969 8.15625 C 122.523438 -2.71875 104.871094 -2.71875 93.980469 8.15625 L 11.535156 90.6875 C 9.199219 92.957031 7.242188 95.585938 5.753906 98.472656 C -4.113281 117.027344 -0.78125 139.832031 13.972656 154.789062 L 235.558594 376.355469 C 253.929688 394.6875 283.675781 394.6875 302.046875 376.355469 L 324.203125 354.207031 L 346.375 376.355469 L 277.421875 445.308594 C 275.09375 447.570312 273.144531 450.203125 271.667969 453.09375 C 261.800781 471.640625 265.125 494.449219 279.886719 509.390625 L 357.449219 586.957031 L 333.097656 611.316406 C 315.214844 603.160156 294.175781 606.671875 279.90625 620.183594 C 271.054688 628.890625 266.117188 640.8125 266.21875 653.226562 C 266.214844 653.296875 266.214844 653.375 266.21875 653.445312 C 266.214844 653.511719 266.214844 653.574219 266.21875 653.648438 C 266.175781 679.1875 286.6875 700.011719 312.238281 700.359375 L 313.542969 700.359375 C 325.859375 700.367188 337.664062 695.445312 346.332031 686.691406 C 360.339844 672.597656 363.957031 651.226562 355.378906 633.304688 L 379.6875 608.984375 L 457.257812 686.570312 C 475.632812 704.871094 505.355469 704.871094 523.734375 686.570312 L 590.222656 620.082031 L 612.390625 642.230469 L 587.765625 666.847656 C 585.433594 669.085938 583.488281 671.6875 582.007812 674.5625 C 572.125 693.105469 575.449219 715.921875 590.222656 730.875 L 811.75 952.476562 C 830.105469 970.839844 859.882812 970.839844 878.246094 952.476562 L 947.160156 883.566406 C 949.496094 881.296875 951.441406 878.667969 952.941406 875.78125 C 962.773438 857.242188 959.449219 834.460938 944.710938 819.515625 Z M 281.152344 350.121094 C 278.230469 353.050781 274.269531 354.6875 270.140625 354.6875 C 266 354.6875 262.042969 353.050781 259.121094 350.121094 L 38.929688 129.910156 C 32.855469 123.824219 32.855469 113.964844 38.929688 107.890625 L 104.984375 41.839844 C 107.90625 38.921875 111.867188 37.273438 116.003906 37.273438 C 120.128906 37.273438 124.101562 38.921875 127.019531 41.839844 L 347.222656 262.042969 C 353.296875 268.121094 353.296875 277.976562 347.222656 284.0625 Z M 316.488281 662.441406 C 315.65625 662.660156 314.808594 662.808594 313.960938 662.886719 C 309.910156 662.871094 306.046875 661.269531 303.191406 658.414062 C 300.203125 655.453125 298.578125 651.398438 298.703125 647.199219 C 298.664062 643.132812 300.277344 639.242188 303.191406 636.394531 C 307.714844 632.109375 314.3125 630.835938 320.105469 633.109375 C 322.019531 633.835938 323.753906 634.964844 325.203125 636.414062 C 326.667969 637.878906 327.828125 639.621094 328.609375 641.550781 C 329.359375 643.414062 329.742188 645.402344 329.75 647.417969 C 329.75 651.542969 328.117188 655.511719 325.203125 658.425781 C 322.78125 660.632812 319.730469 662.035156 316.488281 662.441406 Z M 501.355469 658.398438 C 495.277344 664.472656 485.425781 664.472656 479.335938 658.398438 L 303.191406 482.246094 C 297.101562 476.164062 297.101562 466.308594 303.191406 460.230469 L 457.335938 306.078125 C 463.414062 299.996094 473.269531 299.996094 479.355469 306.078125 L 655.488281 482.246094 C 661.566406 488.328125 661.566406 498.183594 655.488281 504.265625 Z M 919.734375 856.59375 L 853.691406 922.644531 C 850.773438 925.570312 846.804688 927.210938 842.675781 927.210938 C 838.539062 927.210938 834.578125 925.570312 831.65625 922.644531 L 611.449219 702.445312 C 605.375 696.367188 605.375 686.503906 611.449219 680.425781 L 677.523438 614.363281 C 680.445312 611.4375 684.402344 609.792969 688.542969 609.792969 C 692.671875 609.792969 696.640625 611.4375 699.554688 614.363281 L 919.734375 834.578125 C 925.816406 840.664062 925.816406 850.515625 919.734375 856.59375 Z M 919.734375 856.59375 " style={{stroke: 'none', fillRule: 'nonzero',  fillOpacity: 1}} />
                                  <path d="M 446.304688 471.234375 L 468.316406 449.210938 L 512.367188 493.253906 L 490.351562 515.269531 Z M 446.304688 471.234375 " style={{stroke: 'none', fillRule: 'nonzero',  fillOpacity: 1}} />
                                  <path d="M 413.289062 438.210938 L 435.308594 416.195312 L 457.328125 438.210938 L 435.308594 460.230469 Z M 413.289062 438.210938 " style={{stroke: 'none', fillRule: 'nonzero',  fillOpacity: 1}} />
                                  <path d="M 501.355469 526.285156 L 523.382812 504.265625 L 545.390625 526.289062 L 523.371094 548.308594 Z M 501.355469 526.285156 " style={{stroke: 'none', fillRule: 'nonzero',  fillOpacity: 1}} />
                                  <path d="M 314.289062 849.824219 C 202.496094 849.824219 111.867188 759.195312 111.867188 647.402344 C 111.867188 638.808594 104.890625 631.832031 96.296875 631.832031 C 87.695312 631.832031 80.730469 638.808594 80.730469 647.402344 C 80.730469 776.398438 185.292969 880.960938 314.289062 880.960938 C 322.882812 880.960938 329.859375 873.996094 329.859375 865.394531 C 329.859375 856.800781 322.882812 849.824219 314.289062 849.824219 Z M 314.289062 849.824219 " style={{stroke: 'none', fillRule: 'nonzero',  fillOpacity: 1}} />
                                  <path d="M 314.195312 787.511719 C 277.019531 787.539062 241.359375 772.777344 215.066406 746.496094 C 188.789062 720.207031 174.03125 684.542969 174.058594 647.375 C 174.058594 638.777344 167.085938 631.800781 158.488281 631.800781 C 149.890625 631.800781 142.914062 638.777344 142.914062 647.375 C 142.886719 692.808594 160.925781 736.386719 193.046875 768.511719 C 225.183594 800.636719 268.761719 818.675781 314.195312 818.652344 C 322.789062 818.652344 329.765625 811.683594 329.765625 803.078125 C 329.765625 794.480469 322.789062 787.511719 314.195312 787.511719 Z M 314.195312 787.511719 " style={{stroke: 'none', fillRule: 'nonzero',  fillOpacity: 1}} />
                                  <path d="M 314.195312 731.378906 L 313.753906 731.378906 C 270.960938 731.195312 236.390625 696.433594 236.433594 653.648438 C 236.46875 645.046875 229.53125 638.046875 220.929688 638.007812 C 212.332031 637.964844 205.332031 644.910156 205.292969 653.511719 C 205.179688 713.5625 253.75 762.339844 313.808594 762.503906 L 314.289062 762.503906 C 322.882812 762.503906 329.859375 755.53125 329.859375 746.933594 C 329.859375 738.335938 322.882812 731.359375 314.289062 731.359375 Z M 314.195312 731.378906 " style={{stroke: 'none', fillRule: 'nonzero',  fillOpacity: 1}} />
                                </g>
                              </svg>
                              <div>
                              Satellite
                              </div>
                            </a>

                            <a href="javascript:;" className={classes.iconBtn}>
                              <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" height="20pt" width="25pt" style={{fill:vehicle.speed > 80 ? 'red':'green',enableBackground: 'new 0 0 512 512'}} xmlSpace="preserve">
                                <g>
                                  <g>
                                    <path d="M437.019,90.897C388.668,42.545,324.38,15.917,256,15.917S123.332,42.545,74.981,90.897S0,203.536,0,271.917
                      c0,73.315,31.526,143.239,86.495,191.842c3.145,2.782,7.951,2.486,10.733-0.66s2.487-7.952-0.66-10.733
                      c-51.705-45.718-81.359-111.489-81.359-180.449c0-64.317,25.046-124.786,70.526-170.266S191.683,31.125,256,31.125
                      s124.786,25.046,170.266,70.526s70.526,105.948,70.526,170.266c0,68.961-29.654,134.731-81.36,180.448
                      c-3.147,2.782-3.442,7.587-0.66,10.733c1.503,1.7,3.597,2.567,5.7,2.567c1.79,0,3.587-0.629,5.034-1.907
                      C480.474,415.156,512,345.232,512,271.917C512,203.536,485.371,139.249,437.019,90.897z" />
                                  </g>
                                </g>
                                <g>
                                  <g>
                                    <path d="M357.664,426.923H187.206c-4.199,0-7.604,3.405-7.604,7.604s3.405,7.604,7.604,7.604h170.457
                      c5.993,0,10.869,4.876,10.869,10.869v17.009c0,5.993-4.876,10.868-10.869,10.868H154.335c-5.993,0-10.868-4.876-10.868-10.868
                      v-17.009c0-5.993,4.876-10.869,10.868-10.869h7.525c4.199,0,7.604-3.405,7.604-7.604s-3.405-7.604-7.604-7.604h-7.525
                      c-14.379,0-26.076,11.698-26.076,26.077v17.009c0,14.379,11.698,26.075,26.076,26.075h203.329
                      c14.379,0,26.077-11.698,26.077-26.075v-17.009C383.74,438.621,372.043,426.923,357.664,426.923z" />
                                  </g>
                                </g>
                                <g>
                                  <g>
                                    <path d="M385,99.503c-37.458-28.07-82.066-42.907-129.004-42.907c-86.616,0-164.451,51.525-198.293,131.269
                      c-1.533,3.612-0.066,7.798,3.387,9.662c1.145,0.618,2.384,0.913,3.607,0.913c2.467,0,4.873-1.201,6.33-3.382
                      c32.83-49.163,87.668-78.514,146.692-78.514c42.979,0,84.375,15.643,116.562,44.047c3.149,2.778,7.957,2.479,10.733-0.67
                      c2.779-3.149,2.479-7.954-0.669-10.733c-34.968-30.858-79.939-47.852-126.625-47.852c-38.538,0-75.436,11.521-106.484,32.306
                      c37.06-38.818,88.95-61.837,144.762-61.837c43.622,0,85.078,13.786,119.883,39.869c3.361,2.518,8.126,1.835,10.646-1.525
                      C389.043,106.788,388.361,102.021,385,99.503z" />
                                  </g>
                                </g>
                                <g>
                                  <g>
                                    <path  d="M424.472,137.818c-2.618-3.284-7.4-3.825-10.686-1.208c-3.284,2.618-3.823,7.401-1.206,10.686
                      c26.896,33.748,41.817,74.063,43.392,117.017h-48.898c-3.202-21.491-9.993-42.09-20.227-61.323
                      c-1.972-3.707-6.575-5.115-10.285-3.141c-3.708,1.973-5.114,6.577-3.142,10.285c10.398,19.542,16.933,40.617,19.426,62.638
                      c0.435,3.845,3.687,6.749,7.555,6.749h63.311c4.201,0,7.605-3.405,7.605-7.604C471.318,222.643,455.118,176.272,424.472,137.818z" />
                                  </g>
                                </g>
                                <g>
                                  <g>
                                    <path d="M411.889,103.465c-3.293-2.163-7.692-1.454-10.137,1.635L269.802,271.776c-2.607,3.293-2.051,8.074,1.242,10.682
                      s8.075,2.05,10.682-1.242l78.719-99.435l-88.971,165.696c-0.673,1.103-1.826,2.898-2.388,3.603
                      c-2.782,3.496-6.759,5.698-11.199,6.202c-4.439,0.507-8.809-0.751-12.305-3.534c-3.496-2.783-5.698-6.76-6.202-11.2
                      c-0.504-4.439,0.751-8.809,3.548-12.321l23.782-30.041c2.606-3.293,2.05-8.076-1.243-10.683c-3.292-2.606-8.075-2.05-10.682,1.242
                      l-23.768,30.025c-5.314,6.673-7.709,15.016-6.748,23.492c0.962,8.476,5.167,16.07,11.841,21.382
                      c5.709,4.545,12.639,6.956,19.829,6.956c1.215,0,2.438-0.069,3.663-0.208c8.476-0.962,16.07-5.168,21.382-11.84
                      c1.4-1.759,3.242-4.775,3.598-5.362c0.067-0.112,0.133-0.225,0.194-0.34l129.637-241.433
                      C416.276,109.946,415.181,105.627,411.889,103.465z" />
                                  </g>
                                </g>
                                <g>
                                  <g>
                                    <path d="M263.332,334.888c-3.724-5.667-12.867-3.622-13.839,3.099c-0.462,3.196,1.192,6.474,4.106,7.905
                      c2.821,1.386,6.255,0.931,8.574-1.205C264.877,342.198,265.398,337.939,263.332,334.888z" />
                                  </g>
                                </g>
                              </svg>
                              <div>
                                Speed
                              </div>
                            </a>
                        </div>    
              
            </React.Fragment>
          )}
          {activeTabId === 1 && (
            <React.Fragment>
               <HistoryForm vehicle={props.vehicle}/>
            </React.Fragment>
          )}
        </div>

      </div>
    </Grid>
  );
}

export default withRouter(connect(store=>({vehicle:store.liveVehicle}),{setMapView})(Vehicleinfo))


