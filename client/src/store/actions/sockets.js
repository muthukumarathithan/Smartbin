...
import {SET_LIVE_VEHICLES, SET_LIVE_VEHICLE, SET_VEHICLE_STATUS, REMOVE_SOCKET, SET_VEHICLE_ADDRESS} from '../actionTypes';
...
export const setVehicleAddress = (address) => {
    return {
...
export const getVehicleStatus = (ignition_status, speed, device_time, garage) =>{
    ...
    var offline_time =  15/60 * 60;
    var endTime=moment();
    var startTime=moment(device_time);
    var duration = moment.duration(endTime.diff(startTime));
    var minutes = duration.asMinutes();
    if(device_time === undefined)
      return 'offroad';
    else if(garage)
      return 'garage';
...
export const getLiveAddress = (data) =>{
    return async dispatch =>{
        try {
            const address = await api.call('post','address', data);
            dispatch(setVehicleAddress(address));
            dispatch(removeError());
...
export const removeSocket = () =>{
    return async (dispatch, getState) =>{
        try {
            socket.disconnect();
            dispatch(setLiveVehicle({}));
            dispatch(setLiveVehicles([]));
            dispatch(setVehicleStatus({}));
            dispatch(removeError());
